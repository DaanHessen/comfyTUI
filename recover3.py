...
def apply_change(content, target, replacement, start_line, end_line, step_idx):
    lines = content.split('\n')
    ...
    if target in search_area:
        ...
        print(f"FAILED TO MATCH AT STEP {step_idx}: \n--TARGET--\n{target}\n--SEARCH AREA--\n{search_area}\n")
        # Try to find it anywhere
        if content.count(target) == 1:
            print("Found EXACTLY ONE occurrence globally, replacing it.")
            return content.replace(target, replacement, 1)
        print("Multiple or zero occurrences globally!")
        return content
    ...
    for tool_call in step.get("tool_calls", []):
        ...
        if name == "replace_file_content":
            if args.get("TargetFile", "").endswith("main.rs"):
                ...
                content = apply_change(content, target, replacement, sl, el, step_idx)
